...
if __name__ == "__main__":
    ...
    # 16 byte secrets
    # secrets = [''.join([random.choice(string.ascii_letters + string.digits) for _ in range(1)]) for _ in range(NUMBER_OF_LEAVES)]
    secrets = [''.join([random.choice(string.digits) for _ in range(1)]) for _ in range(NUMBER_OF_LEAVES)]
    print("Secrets:", secrets)
    print()

    # 16 byte nullifiers
    # nullifiers = [''.join([random.choice(string.ascii_letters + string.digits) for _ in range(1)]) for _ in range(NUMBER_OF_LEAVES)]
    nullifiers = [''.join([random.choice(string.digits) for _ in range(1)]) for _ in range(NUMBER_OF_LEAVES)]
    print("Nullifiers:", nullifiers)
    print()

    # 32 byte notes
    notes = [n + s for n, s in zip(nullifiers, secrets)]
    # notes = list(range(1,8))
    print("Notes:", notes)
    print()
